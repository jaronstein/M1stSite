...
title: "Your Product Recommendation Slider Is Lying to You"
date: 2026-05-20
...
<form id="throughline-study-form" style="background:var(--mint);border:1px solid var(--accent);border-radius:12px;padding:24px;margin:24px 0;">
  <div style="margin-bottom:16px;">
    <label for="ts-email" style="display:block;font-weight:600;margin-bottom:6px;color:var(--forest);">Your email</label>
    <input type="email" id="ts-email" name="email" required placeholder="you@yourbrand.com" style="width:100%;padding:12px;border:1px solid #ccc;border-radius:6px;font-size:16px;font-family:inherit;box-sizing:border-box;" />
  </div>
  <div style="margin-bottom:16px;">
    <label for="ts-site" style="display:block;font-weight:600;margin-bottom:6px;color:var(--forest);">Your e-commerce site</label>
    <input type="text" id="ts-site" name="ecommerce_site" required placeholder="yourbrand.com" style="width:100%;padding:12px;border:1px solid #ccc;border-radius:6px;font-size:16px;font-family:inherit;box-sizing:border-box;" />
  </div>
  <button type="submit" style="background:var(--forest);color:#fff;border:0;border-radius:6px;padding:14px 24px;font-size:16px;font-weight:600;cursor:pointer;font-family:inherit;">Count me in</button>
  <p id="ts-success" style="display:none;margin:16px 0 0;color:var(--teal);font-weight:600;">Got it. We'll be in touch shortly, probably faster than you expect.</p>
</form>

<script>
(function(){
  var form = document.getElementById('throughline-study-form');
  if (!form) return;
  form.addEventListener('submit', function(e){
    e.preventDefault();
    var email = document.getElementById('ts-email').value;
    var site = document.getElementById('ts-site').value;
    ...
    fetch('https://api.web3forms.com/submit', {
      ...
      body: JSON.stringify({
        ...
        subject: 'Throughline study signup',
        email: email,
        ecommerce_site: site,
        source: 'product-recommendation-sliders-are-lying-to-you'
      })
    }).then(function(){
      form.querySelectorAll('input, button').forEach(function(el){ el.style.display = 'none'; });
      document.getElementById('ts-success').style.display = 'block';
    }).catch(function(){
      btn.disabled = false;
      btn.textContent = 'Count me in';
      alert('Something went wrong. Try again or email justin@mobile1st.com directly.');
    });
  });
})();
</script>
